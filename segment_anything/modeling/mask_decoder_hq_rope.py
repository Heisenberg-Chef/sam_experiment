...
from utils.RotaryEmbedding import RotaryEmbedding
from .common import LayerNorm2d


class MaskDecoderHQRoPE(nn.Module):
    def __init__(
            self,
            *,
            transformer_dim: int,
        ...
        self.iou_token_rope = nn.Parameter(torch.randn([1, transformer_dim]))
    ...
    def forward(
        ...
        rope_generator = RotaryEmbedding(256, device="cuda")
        image_pe = rope_generator(4096, 256)
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token_rope, self.mask_tokens.weight, self.hf_token.weight], dim=0)
